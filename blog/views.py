...
def add_subscriber(request):
    if request.method == "POST":
        ...
        form = SubscriberForm()
    return render(request, "subscription/activate.html", {"form": form})


def remove_subscriber(request):
    if request.method == "POST":
        form = SubscriberForm(request.POST)
        if form.is_valid() and Subscriber.objects.filter(email=form.cleaned_data["email"]):
            sub.delete()
            messages.success(request, "Your email address has been successfully removed from our mailing list.")
            return redirect("post_list")
        else:
            messages.error(request, "This email address does not appear in our mailing list.")
    else:
        form = SubscriberForm()
    return render(request, "subscription/cancel.html", {"form": form})
